...
function Band({
  ...
  useFrame((state, delta) => {
    if (dragged) {
      ...
      (card as any).current?.setNextKinematicTranslation({ x: vec.x - dragged.x, y: vec.y - dragged.y, z: vec.z - dragged.z });
    }
    if ((fixed as any).current) {
      [j1, j2].forEach((ref: any) => {
        if (!ref.current.lerped) ref.current.lerped = new THREE.Vector3().copy(ref.current.translation());
        const clampedDistance = Math.max(0.1, Math.min(1, ref.current.lerped.distanceTo(ref.current.translation())));
        ref.current.lerped.lerp(ref.current.translation(), delta * (minSpeed + clampedDistance * (maxSpeed - minSpeed)));
      });
      curve.points[0].copy((j3 as any).current.translation());
      curve.points[1].copy((j2 as any).current.lerped);
      curve.points[2].copy((j1 as any).current.lerped);
      curve.points[3].copy((fixed as any).current.translation());
      (band.current.geometry as any).setPoints(curve.getPoints(isMobile ? 16 : 32));
      ang.copy((card as any).current.angvel());
      rot.copy((card as any).current.rotation());
      (card as any).current.setAngvel({ x: ang.x, y: ang.y - rot.y * 0.25, z: ang.z });
    }
  });
  ...
  return (
    <>
        ...
        <RigidBody position={[2, 0, 0]} ref={card} {...segmentProps} type={dragged ? 'kinematicPosition' : 'dynamic'}>
          ...
          <group
            ...
            onPointerDown={(e: any) => (
              ...
              drag(new THREE.Vector3().copy(e.point).sub(vec.copy((card as any).current.translation())))
            )}
          >
      ...
      <mesh ref={band}>
        {/* @ts-ignore R3F extend adds meshLineGeometry */}
        <meshLineGeometry />
        {/* @ts-ignore R3F extend adds meshLineMaterial */}
        <meshLineMaterial
          color="white"
          depthTest={false}
